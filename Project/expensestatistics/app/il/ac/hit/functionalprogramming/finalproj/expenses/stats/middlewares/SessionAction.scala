package il.ac.hit.functionalprogramming.finalproj.expenses.stats.middlewares
 ...
 */
@Singleton
class SessionAction @Inject()(playBodyParsers: PlayBodyParsers, wsClient: WSClient, config: Configuration)
    ...
    if (bearerToken.isEmpty || bearerToken.get.trim.isEmpty) {
      ...
      val isAuthRequest: WSRequest =
        wsClient
          ...
          .addHttpHeaders((HttpHeaders.ACCEPT, "application/json"))
          .addHttpHeaders((HttpHeaders.AUTHORIZATION, bearerToken.get))
      ...
      if (response.status == HttpResponseStatus.OK.code()) {
        ...
        block(SessionRequest(userAuthInfo.get.userId, userAuthInfo.get.userEmail, bearerToken.get, request))
      } else {
        Future.successful {
